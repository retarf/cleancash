...
export const CleaningList = () => {
    ...
    return (
        <React.Fragment>
            ...
            {cleaningList.isLoading && childList.isLoading && fieldList.isLoading && salaryList.isLoading &&(
                <Spinner/>
            )}
            {cleaningList.isError && (
                <ErrorBox msg={cleaningList.error instanceof Error && cleaningList.error.message}/>)}
            {childList.isError && (<ErrorBox msg={childList.error instanceof Error && childList.error.message}/>)}
            {fieldList.isError && (<ErrorBox msg={fieldList.error instanceof Error && fieldList.error.message}/>)}
            {salaryList.isError && (<ErrorBox msg={salaryList.error instanceof Error && salaryList.error.message}/>)}
                    ...
                    <Table size="small">
                        ...
                        <TableBody>
                            {cleaningList.data.data.map((cleaning, idx) => {
                                return <CleaningTableRow key={idx} item={cleaning} query={query} baseUrl={APP_ROUTES.CLEANINGS.LIST} />
                            })}
                            <AddButtonRow
                                key="Add"
                                name="Add"
...
const CleaningTableRow = (props) => {
    ...
    >
        ...
        <TableCell>
            {findNameById(childList.data.data, item.child)}
        </TableCell>
        <TableCell>
            <ul>
